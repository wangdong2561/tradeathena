...
| **XAU/USD** 黄金 | OKX XAU-USDT-SWAP WebSocket | 实时推送 | ~100ms |
| **XAG/USD** 白银 | OKX XAG-USDT-SWAP WebSocket | 实时推送 | ~100ms |

> 所有品种均通过 OKX WebSocket 实时推送，tick 级更新，对标 MT4/MT5。
...
| 多周期双图 | ✅ | 左右并排，独立选周期，时间轴联动 |
| K线配色切换 | ✅ | 多套配色方案一键切换 |
| 实时推送 | ✅ | Binance WS + WebSocket 直推前端 |
| K线颜色区分 | ✅ | 形成中: 青/紫 已收定: 蓝/红 |
| 做多/做空 | ✅ | Rust 引擎逐笔模式，多单独立显示 |
| 市价单/限价单/止损单 | ✅ | 三类型订单 |
| 默认止损 | ✅ | 每笔交易强制 -10% 止损 |
| 止损止盈 SL/TP | ✅ | 百分比快速设置 + 手动输入 |
| 实时盈亏计算器 | ✅ | 持仓实时盈亏，开仓时间，逐笔显示 |
| EMA20/50 均线 | ✅ | 可切换显示/隐藏 |
| 金叉/死叉检测 | ✅ | 自动标注 ◆ 标记 + 计数 |
| RSI 指标 | ✅ | 14周期，超买超卖线 |
| 布林带 (Bollinger Bands) | ✅ | 20,2 上/中/下三线显示 |
| 200 EMA | ✅ | 长期趋势线，可切换显示 |
| 策略: 布林带+RSI | ✅ | 图表买卖信号，参数可调 |
| 策略面板 | ✅ | 实时状态监控 + 参数配置 |
| 水平线画线 | ✅ | 点击放置，可清除 |
| 进出场箭头 | ✅ | 自动标注开仓/平仓 |
...
可配置的量化策略，图表上自动生成买卖信号。

### 进场条件

| 信号 | 条件1 | 条件2 | 可选过滤 |
|:---|:---|:---|:---|
| **🟢 做多** | 收盘价 ≤ 布林下轨 | RSI < 30 (超卖) | 价格在 EMA200 上方 |
| **🔴 做空** | 收盘价 ≥ 布林上轨 | RSI > 70 (超买) | 价格在 EMA200 下方 |
...
### 可配置参数
- BB 周期（默认20）& 标准差（默认2）
- RSI 超卖阈值（默认30）& 超买阈值（默认70）
- EMA200 趋势过滤开关
- 止损比例（默认 1.5%）

信号以箭头显示在图表上。打开终端面板的 **「策略」** Tab 可查看实时状态和调整参数。

---

## 品种交易时间 (北京时间 CST UTC+8)

> 所有时间均为北京时间（CST，UTC+8）。

| 品种 | 交易所 | 交易时间 | 休市 |
|:---|:---|:---|:---|
| **BTC/USDT** | Binance/OKX | 🟢 24h × 7天 | 无休市 |
| **XAU/USD** | COMEX | 🟡 周一11:00→周六10:00(夏令时) | 每日10:00-11:00 |
| **XAG/USD** | COMEX | 🟡 周一11:00→周六10:00(夏令时) | 每日10:00-11:00 |
...
如果这个项目对你有帮助，可以请作者喝杯奶茶(¥8-10) ☕

![微信支付收款码 — 打开微信扫一扫即可打赏](pay.jpg)

> 💡 **关于此二维码：** 这是 **微信支付** 收款码。打开手机微信 → 点击右上角"+" → 选择"扫一扫" → 对准上方图片即可打赏。微信是中国最主流的移动支付应用。
